...
export function graphView(stats,recent){
  return `<div class="app-shell">${sidebar('graph',recent)}<main class="content-shell">${topbar()}<section class="main-card graph-page">
    ...
    <div class="graph-layout"><div class="graph-stage" id="graphStage"><canvas id="graphCanvas"></canvas><div class="graph-tooltip" id="graphTooltip"></div><div class="graph-help">Left-click to select · Hold left-click to drag a node · Right-click and drag to move the map · Scroll to zoom</div><div class="graph-legend">${Object.values(RELATIONSHIPS).map((value)=>`<span><i style="background:${value.color}"></i>${escapeHtml(value.label)}</span>`).join('')}</div></div><aside class="graph-detail" id="graphDetail"></aside></div>
...
function placeNodes(graph,edgeMap){
  ...
  entries.forEach(([,nodes],clusterIndex)=>{
    ...
    [...nodes].sort((a,b)=>(edgeMap.get(b.id)?.length||0)-(edgeMap.get(a.id)?.length||0)).forEach((node,index)=>{
      ...
      node.floatPhase=index*1.73+clusterIndex*.91;
      node.floatSpeed=.00042+(index%5)*.000035;
      node.floatAmount=2.2+(index%4)*.55;
    });
  });
}
...
export function mountCoreGraph(records){
  ...
  graph.nodes.forEach((node)=>{
    ...
    image.onload=()=>draw(lastFrameTime);
  ...
  let leftPressNode=null,leftPointerId=null,leftStart=null,leftCurrent=null,leftDragging=false,leftHoldTimer=null;
  ...
  const floatingPoint=(node,time=lastFrameTime)=>{
    const base=worldToScreen(node.x,node.y);
    if(reduceMotion||node===leftPressNode&&leftDragging)return base;
    const phase=node.floatPhase||0;
    const speed=node.floatSpeed||.00045;
    const amount=node.floatAmount||2.5;
    return {x:base.x+Math.sin(time*speed+phase)*amount,y:base.y+Math.cos(time*speed*.83+phase)*amount*.72};
  };

  function updateCursor(){canvas.classList.toggle('is-over-node',Boolean(hovered)&&!rightPanning&&!leftDragging);canvas.classList.toggle('is-right-panning',rightPanning);canvas.classList.toggle('is-node-dragging',leftDragging);}
  function resize(){const rect=stage.getBoundingClientRect();width=Math.max(320,rect.width);height=Math.max(420,rect.height);ratio=Math.min(2,window.devicePixelRatio||1);canvas.width=Math.round(width*ratio);canvas.height=Math.round(height*ratio);canvas.style.width=`${width}px`;canvas.style.height=`${height}px`;ctx.setTransform(ratio,0,0,ratio,0,0);draw(lastFrameTime);}
  ...
  function draw(time=lastFrameTime){lastFrameTime=time;ctx.clearRect(0,0,width,height);ctx.save();ctx.lineCap='round';const connected=selected?new Set((edgeMap.get(selected.id)||[]).map(({other})=>other.id)):null;visibleEdges().forEach((edge)=>{const from=nodeById.get(edge.from);const to=nodeById.get(edge.to);if(!from||!to)return;const a=floatingPoint(from,time);const b=floatingPoint(to,time);const relation=graph.relationships[edge.relationship]||{color:'#aaa'};const active=selected&&(selected.id===from.id||selected.id===to.id);ctx.globalAlpha=selected?(active?.78:.05):.2;ctx.strokeStyle=relation.color;ctx.lineWidth=active?2:.9;ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();});graph.nodes.forEach((node)=>{const point=floatingPoint(node,time);const matches=!searchTerm||node.name.toLowerCase().includes(searchTerm);const isConnected=!selected||selected.id===node.id||connected?.has(node.id);ctx.globalAlpha=matches&&isConnected?1:.12;const radius=Math.max(11,node.radius*Math.max(.72,Math.min(1.25,zoom)));drawNodeIcon(node,point,radius);if(hovered?.id===node.id&&selected?.id!==node.id){ctx.strokeStyle='#111';ctx.lineWidth=2;ctx.beginPath();ctx.arc(point.x,point.y,radius+2,0,Math.PI*2);ctx.stroke();}if(zoom>.86||selected?.id===node.id||hovered?.id===node.id||(searchTerm&&matches)){ctx.globalAlpha=matches&&isConnected?.96:.14;ctx.fillStyle='#171717';ctx.font=`${selected?.id===node.id?700:600} ${Math.max(10,Math.min(13,11*zoom))}px Inter, sans-serif`;ctx.fillText(node.name,point.x+radius+6,point.y+4);}});ctx.restore();}
  function nodeAt(clientX,clientY){const rect=canvas.getBoundingClientRect();const x=clientX-rect.left;const y=clientY-rect.top;let best=null;let distance=Infinity;graph.nodes.forEach((node)=>{const point=floatingPoint(node,lastFrameTime);const radius=Math.max(13,node.radius*Math.max(.72,Math.min(1.25,zoom)));const current=Math.hypot(point.x-x,point.y-y);if(current<=radius+5&&current<distance){best=node;distance=current;}});return best;}
  function selectNode(node,center=false){selected=node||null;detail.innerHTML=node?detailHtml(node,graph,edgeMap):emptyDetail();detail.querySelectorAll('[data-graph-node]').forEach((button)=>button.addEventListener('click',()=>selectNode(nodeById.get(button.dataset.graphNode),true)));if(node&&center){panX=-node.x*zoom;panY=-node.y*zoom;}draw(lastFrameTime);}
  function fitGraph(){if(!graph.nodes.length)return;const xs=graph.nodes.map((node)=>node.x);const ys=graph.nodes.map((node)=>node.y);const minX=Math.min(...xs),maxX=Math.max(...xs),minY=Math.min(...ys),maxY=Math.max(...ys);const spanX=Math.max(1,maxX-minX),spanY=Math.max(1,maxY-minY);const cx=(minX+maxX)/2,cy=(minY+maxY)/2;zoom=Math.min(1.08,Math.max(.3,Math.min((width-140)/spanX,(height-140)/spanY)));panX=-cx*zoom;panY=-cy*zoom;draw(lastFrameTime);}
  function hideTooltip(){tooltip.classList.remove('is-visible');tooltip.innerHTML='';}
  function showTooltip(node,event){if(!node||rightPanning||leftPressNode){hideTooltip();return;}tooltip.innerHTML=tooltipHtml(node,edgeMap);tooltip.classList.add('is-visible');const stageRect=stage.getBoundingClientRect();const tooltipRect=tooltip.getBoundingClientRect();let left=event.clientX-stageRect.left+16;let top=event.clientY-stageRect.top+16;if(left+tooltipRect.width>stageRect.width-10)left=event.clientX-stageRect.left-tooltipRect.width-16;if(top+tooltipRect.height>stageRect.height-10)top=event.clientY-stageRect.top-tooltipRect.height-16;tooltip.style.left=`${Math.max(10,left)}px`;tooltip.style.top=`${Math.max(10,top)}px`;}
  function beginLeftDrag(){if(!leftPressNode||leftDragging)return;leftDragging=true;hideTooltip();updateCursor();if(leftCurrent){const rect=canvas.getBoundingClientRect();const world=screenToWorld(leftCurrent.x-rect.left,leftCurrent.y-rect.top);leftPressNode.x=world.x;leftPressNode.y=world.y;}draw(lastFrameTime);}
  function clearLeftPress(){if(leftHoldTimer){clearTimeout(leftHoldTimer);leftHoldTimer=null;}leftPressNode=null;leftPointerId=null;leftStart=null;leftCurrent=null;leftDragging=false;}
  function animate(time){draw(time);animationFrame=requestAnimationFrame(animate);}
  ...
  canvas.addEventListener('pointerdown',(event)=>{
    if(event.button===0){
      ...
      leftPressNode=node;leftPointerId=event.pointerId;leftStart={x:event.clientX,y:event.clientY};leftCurrent={...leftStart};leftDragging=false;hideTooltip();
      try{canvas.setPointerCapture(event.pointerId);}catch{}
      leftHoldTimer=setTimeout(beginLeftDrag,HOLD_DELAY);
      updateCursor();return;
    }
  ...
  canvas.addEventListener('pointermove',(event)=>{
    if(leftPressNode&&event.pointerId===leftPointerId){
      leftCurrent={x:event.clientX,y:event.clientY};
      if(leftDragging){const rect=canvas.getBoundingClientRect();const world=screenToWorld(event.clientX-rect.left,event.clientY-rect.top);leftPressNode.x=world.x;leftPressNode.y=world.y;}
      hovered=leftPressNode;hideTooltip();updateCursor();return;
    }
    hovered=nodeAt(event.clientX,event.clientY);
    if(rightPanning&&rightLast){panX+=event.clientX-rightLast.x;panY+=event.clientY-rightLast.y;rightLast={x:event.clientX,y:event.clientY};hideTooltip();}
    else if(hovered)showTooltip(hovered,event);else hideTooltip();
    updateCursor();
  });
  canvas.addEventListener('pointerup',(event)=>{
    if(event.button===0&&leftPressNode&&event.pointerId===leftPointerId){const node=leftPressNode;const wasDragging=leftDragging;clearLeftPress();selectNode(node);try{canvas.releasePointerCapture(event.pointerId);}catch{}hovered=nodeAt(event.clientX,event.clientY);if(hovered&&!wasDragging)showTooltip(hovered,event);updateCursor();return;}
    if(event.button===2||rightPanning){rightPanning=false;rightLast=null;try{canvas.releasePointerCapture(event.pointerId);}catch{}hovered=nodeAt(event.clientX,event.clientY);if(hovered)showTooltip(hovered,event);updateCursor();}
  });
  canvas.addEventListener('pointercancel',()=>{clearLeftPress();rightPanning=false;rightLast=null;hideTooltip();updateCursor();});
  canvas.addEventListener('pointerleave',()=>{if(!rightPanning&&!leftPressNode){hovered=null;hideTooltip();updateCursor();}});
  ...
  canvas.addEventListener('wheel',(event)=>{event.preventDefault();hideTooltip();const rect=canvas.getBoundingClientRect();const x=event.clientX-rect.left;const y=event.clientY-rect.top;const before=screenToWorld(x,y);zoom=Math.max(.28,Math.min(2.4,zoom*(event.deltaY>0?.9:1.1)));panX=x-width/2-before.x*zoom;panY=y-height/2-before.y*zoom;},{passive:false});
  search?.addEventListener('input',()=>{searchTerm=search.value.toLowerCase().trim();const match=searchTerm?(graph.nodes.find((node)=>node.name.toLowerCase()===searchTerm)||graph.nodes.find((node)=>node.name.toLowerCase().startsWith(searchTerm))):null;if(match)selectNode(match,true);});
  relationshipSelect?.addEventListener('change',()=>{relationship=relationshipSelect.value;});
  document.getElementById('graphFit')?.addEventListener('click',fitGraph);
  document.getElementById('graphExport')?.addEventListener('click',()=>{const payload=JSON.stringify({nodes:graph.nodes.map(({x,y,radius,floatPhase,floatSpeed,floatAmount,...node})=>node),edges:graph.edges},null,2);const url=URL.createObjectURL(new Blob([payload],{type:'application/json'}));const link=document.createElement('a');link.href=url;link.download='core-graph.json';link.click();URL.revokeObjectURL(url);});

  const observer=new ResizeObserver(resize);observer.observe(stage);resize();requestAnimationFrame(fitGraph);animationFrame=requestAnimationFrame(animate);updateCursor();
  return()=>{observer.disconnect();if(animationFrame)cancelAnimationFrame(animationFrame);if(leftHoldTimer)clearTimeout(leftHoldTimer);};
}
