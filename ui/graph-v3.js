...
export function mountCoreGraph(records){
  ...
  function drawNodeIcon(node,point,radius){
    ...
    if(image&&image.complete&&image.naturalWidth){
      ...
      ctx.fillStyle=COLORS[node.cluster]||'#888';
      ctx.fillRect(point.x-radius,point.y-radius,radius*2,radius*2);
    }
    ctx.restore();
    ctx.strokeStyle=selected?.id===node.id?'#111':'rgba(255,255,255,.95)';
  ...
  canvas.addEventListener('pointermove',(event)=>{
    ...
    if(rightPanning&&last){panX+=event.clientX-last.x;panY+=event.clientY-last.y;last={x:event.clientX,y:event.clientY};hideTooltip();}
    else if(hovered)showTooltip(hovered,event);
    else hideTooltip();
    updateCursor();draw();
  });
  function endRightPan(event){if(event.button!==2&&!rightPanning)return;rightPanning=false;last=null;try{canvas.releasePointerCapture(event.pointerId);}catch{}hovered=nodeAt(event.clientX,event.clientY);if(hovered)showTooltip(hovered,event);updateCursor();draw();}
  canvas.addEventListener('pointerup',endRightPan);
  canvas.addEventListener('pointercancel',()=>{rightPanning=false;last=null;hideTooltip();updateCursor();draw();});
  canvas.addEventListener('pointerleave',()=>{if(!rightPanning){hovered=null;hideTooltip();updateCursor();draw();}});
